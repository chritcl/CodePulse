...
:style="coreContentStyle">
        <div class="inner-wrapper">
          <transition mode="out-in"
@enter="onInnerEnter" :css="false" @leave="onInnerLeave">
            <div
              v-if="isMsgActive"
              key="msg"
              class="msg-box"
            ...
            >
              <div class="msg-avatar">
                <img
:src="currentMsgIcon" alt="消息图标" class="msg-avatar-img" />
              </div>
            ...
            <div v-else-if="displayHardware"
class="systemstate-box" key="hardware">
              <div class="hw-item">
                <span class="hw-label">CPU</span>
                <span class="hw-value"
:class="{ 'high-usage': parseInt(cpuUsage) >= 90 }">{{
                  cpuUsage
                }}</span>
              </div>
              ...
              <div class="hw-item">
                ...
                <span class="hw-value"
:class="{ 'high-usage': parseInt(gpuUsage) >= 90 }">{{
                  gpuUsage
                }}</span>
              </div>
              <div class="hw-divider" />
              <div class="hw-item">
                ...
                <span class="hw-value"
:class="{ 'high-usage': parseInt(memUsage) >= 90 }">{{
                  memUsage
                }}</span>
              </div>
            </div>
...
fill="currentColor">
            ...
            <div v-else-if="displaySpeed"
class="speed-box" key="speed">
...
@enter="onInnerEnter" :css="false" @leave="onInnerLeave">
          ...
          <div
v-else :class="['status-dot', networkStatus]" key="dot" />
        </transition>
      </div>
    </div>
  </transition>
...
const onEnter = (el: Element, done: () => void) => {
  ...
  const animate = (time: number) => {
    ...
    const opacity = Math.min(1, progress * 4); // 快速淡入
...
const onLeave = (el: Element, done: () => void) => {
  ...
  const animate = (time: number) => {
    ...
    // 离开动画：快速平滑回缩
    // 使用 easing 曲线或简化的衰减
    const scale = 1 - Math.pow(progress, 3); // 快速内收
