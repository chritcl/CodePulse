...
import { useClaudeDisplayPreferences } from './useClaudeDisplayPreferences';

const event = (
  payload: ClaudeDisplayPreferencesPayload
): Event<ClaudeDisplayPreferencesPayload> => ({
  event: 'claude-display-preferences-updated',
  id: 1,
  payload,
});

describe('useClaudeDisplayPreferences', () => {
  it('读取默认偏好并接收主窗口同步的显示更新', async () => {
    localStorage.clear();
    let listener: ((received: Event<ClaudeDisplayPreferencesPayload>) => void) | undefined;
    const listenEvent: EventListen = async (_eventName, registered) => {
      listener = registered as (received: Event<ClaudeDisplayPreferencesPayload>) => void;
      return () => {};
    };
    const preferences = useClaudeDisplayPreferences({ listenEvent });
