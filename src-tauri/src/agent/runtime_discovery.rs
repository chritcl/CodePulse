...
#[serde(rename_all = "camelCase")]
pub struct AgentSummaryCapture {
    #[serde(default)]
    pub codex: bool,
    #[serde(default)]
    pub claude: bool,
...
pub struct AgentRuntimeDiscovery {
    ...
    pub capture_task_summary_by_provider: AgentSummaryCapture,
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct AgentRuntimeDiscoveryWire {
    #[serde(flatten)]
    runtime: RuntimeDiscovery,
    #[serde(default)]
    capture_task_summary_by_provider: Option<AgentSummaryCapture>,
}
...
impl AgentRuntimeDiscovery {
    pub fn validate(&self) -> Result<(), DiscoveryError> {
        self.runtime.validate()
    }

    pub fn capture_task_summary_for(&self, provider: AgentProvider) -> bool {
        match provider {
            AgentProvider::Codex => self.capture_task_summary_by_provider.codex,
            AgentProvider::Claude => self.capture_task_summary_by_provider.claude,
        }
    }
}
...
pub fn read_agent_discovery(path: &Path) -> Result<AgentRuntimeDiscovery, DiscoveryError> {
    let wire = serde_json::from_slice::<AgentRuntimeDiscoveryWire>(&fs::read(path)?)?;
    let capture = wire.capture_task_summary_by_provider.unwrap_or(AgentSummaryCapture {
        codex: wire.runtime.capture_task_summary,
        claude: false,
    });
    let discovery = AgentRuntimeDiscovery {
        runtime: wire.runtime,
        capture_task_summary_by_provider: capture,
    };
    discovery.validate()?;
    Ok(discovery)
}
...
pub fn write_discovery_atomically(
    ...
    discovery.validate()?;
    write_serialized_discovery(path, discovery)
}

pub fn write_agent_discovery_atomically(
    path: &Path,
    discovery: &AgentRuntimeDiscovery,
) -> Result<(), DiscoveryError> {
    let mut discovery = discovery.clone();
    discovery.runtime.capture_task_summary = discovery.capture_task_summary_by_provider.codex;
    discovery.validate()?;
    write_serialized_discovery(path, &discovery)
}

pub fn remove_discovery(path: &Path) -> Result<(), DiscoveryError> {
...
fn write_serialized_discovery(
    path: &Path,
    discovery: &impl Serialize,
) -> Result<(), DiscoveryError> {
    let parent = path
        .parent()
    ...
    let result = write_discovery_file(&temporary_path, discovery)
...
fn write_discovery_file(path: &Path, discovery: &impl Serialize) -> Result<(), DiscoveryError> {
