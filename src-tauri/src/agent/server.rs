...
use crate::claude::protocol::ClaudeBridgeEvent;
use crate::codex::protocol::CodexBridgeEvent;

use super::runtime_discovery::{
    discovery_file_path, remove_discovery, write_agent_discovery_atomically, AgentRuntimeDiscovery,
    AgentSummaryCapture, DiscoveryError, RuntimeDiscovery, RUNTIME_DISCOVERY_VERSION,
};
...
pub enum AgentBridgeEvent {
    Codex(CodexBridgeEvent),
    Claude(ClaudeBridgeEvent),
}
...
impl fmt::Display for ReceiverError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Io(error) => write!(formatter, "Agent 接收器 IO 错误: {error}"),
            Self::Discovery(error) => write!(formatter, "Agent 发现文件错误: {error}"),
            Self::InvalidQueueCapacity => formatter.write_str("Agent 接收器队列容量必须大于零"),
        }
    }
}
...
struct ReceiverState {
    token: Arc<str>,
    sender: mpsc::Sender<AgentBridgeEvent>,
}
...
pub async fn start_receiver(
...
) -> Result<(AgentEventReceiver, mpsc::Receiver<AgentBridgeEvent>), ReceiverError> {
    if queue_capacity == 0 {
    ...
    let discovery = AgentRuntimeDiscovery {
        runtime: RuntimeDiscovery {
            ...
            capture_task_summary: capture.codex,
        },
        capture_task_summary_by_provider: capture,
    };
    write_agent_discovery_atomically(&discovery_path, &discovery)?;

    let (sender, receiver) = mpsc::channel(queue_capacity);
    let state = ReceiverState {
        token: Arc::from(discovery.runtime.token),
        sender,
    };
...
fn parse_event(body: &[u8]) -> Option<AgentBridgeEvent> {
    let value = serde_json::from_slice::<Value>(body).ok()?;
    match value.get("provider").and_then(Value::as_str) {
        None | Some("codex") => serde_json::from_value::<CodexBridgeEvent>(value)
            .ok()
            .and_then(|event| event.sanitize_and_validate().ok())
            .map(AgentBridgeEvent::Codex),
        Some("claude") => serde_json::from_value::<ClaudeBridgeEvent>(value)
            .ok()
            .and_then(|event| event.sanitize_and_validate().ok())
            .map(AgentBridgeEvent::Claude),
        Some(_) => None,
    }
}
