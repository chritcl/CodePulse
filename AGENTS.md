...
2. 动态岛状态机、拖拽位置持久化和右键菜单已完成最小版本，并已支持主进程右键菜单打开当前任务、打开当前任务对应 Agent 和稍后提醒当前任务；复制摘要等更多任务级菜单动作和更完整的 Windows 手测仍未完成。
