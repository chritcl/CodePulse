...
7. 当前下一优先级为：通知策略细化、更完整动态岛菜单策略、非 Codex 打开 Agent 策略、更完整 Codex 会话识别、多显示器/DPI/Explorer/系统休眠 Windows 手测、更细 UI 失败展示。
...
11. IPC 参数校验、安全测试和任务操作授权最小版本，覆盖任务 ID、数据源 ID、动态岛模式、稍后提醒时间、启用状态、设置更新白名单、数据源启用与设置持久化联动、`settings.update` 启用状态同步 AgentStateHub、Codex/通用日志路径运行期配置同步、自定义命令运行期配置同步、任务打开、打开 Codex Agent、复制摘要、稍后提醒和标记已读的真实任务校验、Preload API 白名单冻结、状态订阅清理和诊断导出脱敏。
12. Windows 通知最小版本，包含任务完成、任务失败、等待处理、长时间无活动、数据源断开、额度不足和额度耗尽通知，以及事件去重、点击跳转、稍后提醒、勿扰时间段、通知去重记录清理、过期稍后提醒清理和通知运行时诊断统计。
13. 动态岛状态机、拖拽位置持久化、右键菜单、全屏自动隐藏、显示器变化处理和系统休眠恢复最小版本，包含任务开始、阶段变化、任务完成、额度阈值跨越触发展开，等待、失败、额度耗尽、数据源断开持续提醒，阶段变化节流、自动收起、悬停暂停、滚轮切换任务、Esc 收起、稍后提醒、顶部拖拽区域、自定义坐标保存、工作区限制、显示器断开回落，多显示器热插拔和 DPI 指标变化后重新限制动态岛位置并关闭贴边弹窗，系统休眠前关闭贴边弹窗、恢复后重算动态岛位置并刷新状态，全屏应用出现时按设置隐藏并在退出全屏后恢复，以及展开、收起、隐藏、打开任务中心和设置入口。
14. 主任务中心交互补强最小版本，包含关键词搜索、状态筛选、数据源筛选、项目筛选、时间筛选、当前快照历史分组、持久化历史查询、活动时间线、错误详情、项目路径展示、任务摘要复制到剪贴板，以及通过主进程受控打开 Codex Agent 和项目目录。
...
16. 开发、打包、适配器扩展、故障排查和产品架构文档。
17. 构建产物已可通过 `pnpm run dist` 生成 Windows NSIS 安装包。
...
2. 动态岛状态机、拖拽位置持久化和右键菜单已完成最小版本；更完整的菜单策略、主进程级事件菜单和更完整的手测仍未完成。
3. Windows 通知已完成最小版本，但原生通知动作在不同 Windows 版本上的手测和更细的勿扰策略仍需加强。
4. 主任务中心已完成搜索、状态筛选、数据源筛选、项目筛选、时间筛选、当前快照历史分组、持久化历史查询、活动时间线、错误详情、项目路径展示、复制摘要、打开 Codex Agent 和打开项目目录最小版本；非 Codex 打开 Agent 策略和更完整运行统计仍未完成。
5. 设置页仍为基础页，已包含 Codex 状态源、Codex 日志源、通用 Agent 日志源路径入口、Codex/本机进程/通用日志/模拟/自定义命令启用开关、自定义命令基础配置、Codex/通用日志路径运行期配置同步和自定义命令运行期配置同步；尚未完成显示器选择、动态岛位置细项和完整勿扰策略。
6. SQLite 已完成任务历史和活动记录最小落库、任务中心历史查询、数据库损坏恢复和清理策略；诊断数据落库和更细 UI 失败展示尚未完成。
7. 全屏应用自动隐藏、Explorer 重启托盘恢复、多显示器热插拔、DPI 动态变化和系统休眠恢复已完成最小版本；多显示器、DPI、Explorer 重启和系统休眠恢复仍需 Windows 手测和更细 UI 失败展示。
...
7. 部分完成：完整体验已实现 Windows 通知最小版本、通知运行时记录清理、主任务中心交互补强最小版本、Codex 打开 Agent 最小版本、全屏自动隐藏最小版本、Explorer 托盘恢复最小版本、多显示器热插拔和 DPI 动态变化最小版本、系统休眠恢复最小版本、SQLite 历史持久化、任务中心历史查询、数据库损坏恢复、历史清理、数据源启用设置联动、设置页启用开关、Codex/通用日志路径运行期配置同步和自定义命令运行期配置同步最小版本；多显示器/DPI/Explorer/系统休眠 Windows 手测和更细 UI 失败展示仍未完成。
