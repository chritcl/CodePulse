...
7. 当前下一优先级为：通知策略细化、更完整动态岛菜单策略、更完整 Codex 会话识别、多显示器/DPI/Explorer/系统休眠 Windows 手测和更细 UI 失败展示。
...
14. 主任务中心交互补强最小版本，包含关键词搜索、状态筛选、数据源筛选、项目筛选、时间筛选、当前快照历史分组、持久化历史查询、活动时间线、错误详情、项目路径展示、任务摘要复制到剪贴板、运行统计面板，以及通过主进程受控打开 Codex Agent、非 Codex Agent 项目目录回退和打开项目目录；运行统计覆盖总任务、运行中、等待、失败、完成、历史样本、覆盖数据源、覆盖项目、平均结束耗时、最长无活动任务和最近活动时间。
...
4. 主任务中心已完成搜索、状态筛选、数据源筛选、项目筛选、时间筛选、当前快照历史分组、持久化历史查询、活动时间线、错误详情、项目路径展示、复制摘要、打开 Codex Agent、非 Codex Agent 项目目录回退、打开项目目录和运行统计最小版本；更深入的趋势图、成功率和运行质量分析仍未完成。
