...
1. `CodexAdapter` 已完成可配置状态源和日志源解析、状态源和日志源路径运行期热更新、状态源和日志源多字段会话识别最小版本，`ProcessAdapter` 已完成本机 Agent 进程检测最小版本，`LogAdapter` 已完成通用 JSONL 日志源解析和日志源路径运行期热更新最小版本，`CustomCommandAdapter` 已完成默认禁用、授权执行和授权/命令路径/参数运行期热更新最小版本；更多 Codex 原生日志格式覆盖仍需补强。
...
6. 部分完成：真实接入已实现首期 `CodexAdapter` 进程检测、可配置状态源解析、可配置日志源解析、状态源和日志源多字段会话识别、设置页状态源和日志源路径入口、状态源和日志源路径运行期热更新最小版本，`ProcessAdapter` 本机 Agent 进程检测最小版本，`LogAdapter` 通用 UTF-8 JSONL 日志源解析和日志源路径运行期热更新最小版本，以及 `CustomCommandAdapter` 默认禁用、显式授权、安全执行和授权/命令路径/参数运行期热更新最小版本；解析失败、进程检测失败和命令执行失败时必须继续降级为明确错误状态，更多 Codex 原生日志格式覆盖仍需补强。
