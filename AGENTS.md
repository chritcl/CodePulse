...
7. 当前下一优先级为：通知策略细化、更完整动态岛菜单策略、更完整 Codex 会话识别、多显示器/DPI/Explorer/系统休眠 Windows 手测、更细 UI 失败展示和更完整运行统计。
...
14. 主任务中心交互补强最小版本，包含关键词搜索、状态筛选、数据源筛选、项目筛选、时间筛选、当前快照历史分组、持久化历史查询、活动时间线、错误详情、项目路径展示、任务摘要复制到剪贴板，以及通过主进程受控打开 Codex Agent、非 Codex Agent 项目目录回退和项目目录。
...
4. 主任务中心已完成搜索、状态筛选、数据源筛选、项目筛选、时间筛选、当前快照历史分组、持久化历史查询、活动时间线、错误详情、项目路径展示、复制摘要、打开 Codex Agent、非 Codex Agent 项目目录回退和打开项目目录最小版本；更完整运行统计仍未完成。
...
7. 部分完成：完整体验已实现 Windows 通知最小版本、通知运行时记录清理、主任务中心交互补强最小版本、Codex 打开 Agent 最小版本、非 Codex Agent 项目目录回退最小版本、全屏自动隐藏最小版本、Explorer 托盘恢复最小版本、多显示器热插拔和 DPI 动态变化最小版本、系统休眠恢复最小版本、SQLite 历史持久化、任务中心历史查询、数据库损坏恢复、历史清理、数据源启用设置联动、设置页启用开关、Codex/通用日志路径运行期配置同步和自定义命令运行期配置同步最小版本；多显示器/DPI/Explorer/系统休眠 Windows 手测和更细 UI 失败展示仍未完成。
