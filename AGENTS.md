...
17. 设置页显示器与动态岛位置配置最小版本，包含动态岛位置、固定/跟随活动显示器、目标显示器刷新与选择、全屏自动隐藏和始终置顶。
...
5. 设置页仍为基础页，已包含 Codex 状态源、Codex 日志源、通用 Agent 日志源路径入口、Codex/本机进程/通用日志/模拟/自定义命令启用开关、自定义命令基础配置、通知策略配置、显示器与动态岛位置配置、Codex/通用日志路径运行期配置同步和自定义命令运行期配置同步；尚未完成动态岛自由坐标编辑、显示策略发布级体验和更多设置失败态展示。
