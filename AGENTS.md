...
18. 开发、打包、适配器扩展、故障排查和产品架构文档。
19. 构建产物已可通过 `pnpm run dist` 生成 Windows NSIS 安装包。
...
5. 设置页仍为基础页，已包含 Codex 状态源、Codex 日志源、通用 Agent 日志源路径入口、Codex/本机进程/通用日志/模拟/自定义命令启用开关、自定义命令基础配置、通知策略配置、显示器与动态岛位置配置、动态岛自由坐标编辑、Codex/通用日志路径运行期配置同步和自定义命令运行期配置同步；尚未完成显示策略发布级体验和更多设置失败态展示。
