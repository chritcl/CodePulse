...
- commit message 使用中文；是否添加共同作者由实际执行时根据真实参与者决定。
...
git commit -m "refactor: 统一音乐播放时间锚点契约"
```
...
git commit -m "refactor: 重建音乐播放时间线"
```
...
git commit -m "refactor: 重建歌词解析匹配与缓存"
```
...
```powershell
git add src-tauri
git commit -m "refactor: 建立统一歌词服务"
```
...
git commit -m "refactor: 重建前端歌词状态机"
```
...
```powershell
git add src/composables src/shared/ipc/contracts.ts
git commit -m "refactor: 重建音乐播放器会话"
```
...
```powershell
git add src
git commit -m "refactor: 接入全新歌词与播放进度链路"
```
