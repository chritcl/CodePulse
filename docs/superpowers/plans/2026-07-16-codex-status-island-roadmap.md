...
- 普通事务`StableArtifactLease`固定不含`DELETE`权限，全部持有到`StructureCommitted`成功落盘。`StructureCommitted`后先释放普通Lease，再为每个普通事务产物新开`GENERIC_READ | DELETE`、仅`FILE_SHARE_READ`的cleanup Handle；在新取得的同一 cleanup Handle 上依次读取identity、计算摘要、再次确认identity并标记删除，关闭后才允许路径被复用。禁止给原Lease加`DELETE`、把原Lease复用为cleanup Handle、释放后按路径`DeleteFileW`、先按路径读摘要再重新按路径删除，或因cleanup失败回滚已正确提交的结构。
...
04A static CodexIntegrationInspection + derive_codex_listening_status()
        │                         │
        │ 内存 Hook 变更计划      └──▶ CodexListeningStatus（唯一动态状态）
        ▼
04B CodexIntegrationTransactionJournal
        ...
        │ Prepared持久化 → optimistic precondition check → 创建并验证staging → all-staging-ready
...
`configPath` 是 Hook 表示的主文件；`featureConfigPath` 只用于告诉用户手动启用 Hooks 的配置位置。本地 `[features].hooks=false` 或只有 `codex_hooks=false` 时 install/repair 返回稳定 `HooksDisabled`，但当 representation 可安全解析、markerPresence=present 且 managedEntry 为 exact/modified/duplicate 时，允许 preview/apply uninstall，只精确删除 CodePulse marker；不得安装 Bridge或启动 Runtime。只有旧别名时 UI 额外提示“检测到旧版 codex_hooks 配置，请在 Codex 中改用 hooks。”两个 Feature 键冲突时 install/repair/uninstall 全部返回 ConfigConflict，不产生内存变更计划或Prepared Journal，Runtime RemainStopped，UI 不显示任何变更按钮。managed disabled 对 install/repair/uninstall 全部返回 `ManagedDisabled`；config conflict/ambiguous 不自动卸载。`expectedDigest` 是所有决策输入（`hooks.json`、`config.toml`、只读 `requirements.toml`、Bridge 资源/副本/安装记录）的路径、存在性和原始 SHA-256 经排序后的组合摘要。`previewDigest` 是规范化变更计划的 SHA-256；应用时重新计算两种摘要，任何不一致都停止写入。
...
唯一正式执行顺序为：静态检查 → 展示预览 → 用户确认 → 分配transactionId → pure prepare Config/Bridge/Record且零稳定目标副作用 → 持久化Prepared Journal → 对Config/Bridge/Record执行optimistic precondition check → 创建并验证prepared backup、target temp与removal staging → `all-staging-ready` → Bridge/Record原子目标操作 → 持久化BridgeApplied → 必要时启动临时Runtime → Hook Config原子目标操作 → 持久化ConfigApplied → action-specific invariant → 持久化StructureCommitted → 释放普通事务StableArtifactLease → 使用新取得的cleanup Handle清理普通事务产物 → 发布awaiting_trust/partial/not_installed/service_error → 本地self-check → 等待self-check完成后当前Runtime generation第一条新真实认证Hook → running。某action不涉及的资源可以把相应阶段判定为已满足/无需变更，但不得调换顺序或派生第二套phase。
...
阶段语义固定为，且只有下列四项可以持久化到Journal：

- `Prepared`：用户已经确认，transactionId已分配，事务意图、当前稳定目标的存在性/身份/摘要预条件、目标内容与全部确定性路径已经冻结，pure prepare已经完成；在创建本事务任何正式backup/temp/removal staging、修改Bridge/记录或配置之前持久化。它不表示staging已经完成。
...
`PreconditionChecked`、`Staging`、`AllStagingReady`、`Verifying`、`Cleaning`、`RollingBack`等名称只可作为函数、内存流程或日志描述，不能成为Journal phase。`awaiting_trust`、`partial`、`not_installed`、`service_error`、`running`属于集成/监听状态；`revision`与`generation`属于状态版本事实；它们也不是事务阶段。

既有公开类型名`PreparedCodexHookChange`保持不变，但它只是04A生成的纯内存变更计划，不表示Prepared Journal已经持久化，也不是第五个事务阶段。
...
transactionId 必须在两个 transaction prepare 之前分配。`prepare_config_apply()` 与 `prepare_bridge_install()` 都是pure prepare：只解析/校验输入，读取稳定Config/Bridge/Record，构造目标字节和安装记录，验证打包Bridge的PE/hash/piped契约，计算存在性、身份、摘要与确定性staging/backup/snapshot/tombstone路径，并把事务意图保存在内存；不得创建target temp、prepared backup、snapshot或tombstone，不得替换稳定Bridge/记录/配置、启动Runtime或创建第二个日志。完整Prepared Journal原子持久化成功后，coordinator必须先重新读取Config/Bridge/Record并分别调用`verify_artifact_matches_expected_original()`；只有三者均Original/ExpectedAbsent才允许创建本transactionId的正式staging，Target或ExternalModification均返回`ConcurrentModification`/稳定Conflict并保留Journal，不采用新字节为Original。这个步骤统一称为 **optimistic precondition check**：它验证用户确认时看到的稳定目标仍未改变，只能提前发现修改，绝不是原子CAS，也不能证明检查后目标不会再次变化；它与随后验证事务artifact是否符合计划摘要的staging verification是两个不同步骤。

每个existedBefore=true的prepared backup必须从当前目标重读原字节，摘要先等于Journal.originalDigest，再用`create_new(true)`创建确定性backup，write→flush→close并通过Handle-based identity/摘要验证仍等于originalDigest；失败时零目标替换。existedBefore=false要求prepared backup=None且目标仍缺失，否则ExternalModification。target temp同样须close后通过Handle验证targetDigest；targetExists=false使用已验证“无替换payload”状态且不能以零字节文件冒充缺失。第一个稳定目标原子操作前，Config/Bridge/Record所需staging必须全部创建并验证，所有需要保留的prepared backup以及snapshot/tombstone/capture路径的存在性、归属、同目录/同卷条件已经满足，当前Prepared Journal仍属于同一transactionId；这一个内存屏障统一称为`all-staging-ready`。任一资源未就绪时零稳定目标操作，且不得把屏障持久化为第五个phase。

三种原子语义必须由统一`AtomicIntegrationFs`分别实现，writer与installer不得各自实现替换规则，也不得抽象成一个`move_with_replace(...)`：

1. **Atomic existing-file replacement**：`existed_before=true && target_exists=true`时，只能在统一optimistic precondition check与`all-staging-ready`均已通过后调用`ReplaceFileW(target, replacement_temp, transaction_owned_replaced_snapshot, ...)`；该接口不得再执行第二次optimistic precondition check。成功后的固定顺序是：获取target `StableArtifactLease`→获取replaced snapshot `StableArtifactLease`→通过target Lease计算target digest→通过snapshot Lease计算snapshot digest→验证两个volume serial/file ID组合不同。只有target=`targetDigest`、snapshot=`originalDigest`且两个Lease均成功，才返回`AppliedExpectedOriginal`。任一Lease取得失败返回`ActiveArtifactHandleConflict`，不再按路径读取、不清snapshot、不继续下一文件，保留current target、snapshot、prepared backup与Journal，停止Runtime并发布`IntegrationTransactionConflict`；snapshot摘要稳定但不等于originalDigest才进入late modification分支。
...
3. **Atomic removal capture**：Uninstall删除已有Bridge/Record时不得调用`DeleteFileW`；统一optimistic precondition check与`all-staging-ready`通过后，把目标no-replace原子rename到本transactionId的removed tombstone，不在单文件接口内重跑precondition。成功后固定获取tombstone Lease、通过该Handle计算摘要、在Lease仍持有时验证原目标路径absent。Lease失败时不按路径读tombstone、不永久删除、不进入`Uninstalled` invariant；摘要不符或目标路径重建同样保留全部版本并Conflict。StructureCommitted前不得永久删除tombstone。
...
成功应用一个目标后，target、replacement结果、正常snapshot、tombstone及其他参与事务身份验证的`AppliedArtifactLeases`进入当前进程的统一事务句柄，并全部保持到`StructureCommitted` Journal原子持久化；`BridgeApplied`/`ConfigApplied`只是中间持久阶段，不是普通Lease释放点。阶段推进失败时保持Lease、停止后续操作并进入Conflict，不能先drop再继续。StructureCommitted成功后才释放全部普通事务Lease，并为每个普通artifact新取得cleanup Handle；该cleanup Handle不是原Lease，也不得复用原Lease。进程崩溃自然释放Handle，startup recovery重新获取，任何Lease都不写入Journal。
...
Uninstall 复用同一 Journal和相同阶段顺序：Prepared → 原子捕获Bridge/Record tombstone → BridgeApplied → 原子移除CodePulse marker → ConfigApplied → Uninstalled invariant → StructureCommitted。资源本来缺失时相应原子步骤按“已满足/无需变更”推进，但不得跳过BridgeApplied或把ConfigApplied放到BridgeApplied之前。每个待删目标在统一precondition与`all-staging-ready`后执行no-replace原子rename，随后必须取得tombstone Lease、通过同一Handle验证摘要并确认目标absent；Lease失败不分类、不恢复、不永久删除并进入可重试Conflict。摘要不符时，只有Lease成功且目标路径仍absent才可释放相关Lease后no-replace恢复，操作后再获取restored target Lease验证；目标已被重新创建时保留两者。只有Marker=Absent、配置无稳定路径引用、Bridge/Record目标路径absent且对应tombstone Lease稳定并受Journal控制，才能通过`Uninstalled` invariant并推进StructureCommitted；Runtime停止、Store高revision空快照与not_installed发布属于StructureCommitted后的Runtime编排，不是文件事务phase。之后才通过`delete_verified_artifact_by_handle`清理普通tombstone，失败只Warning且不恢复Hook。

Install、Repair、Uninstall都使用本节开头的唯一顺序。用户确认前不得分配transactionId或进入pure prepare；`prepare_config_apply`和`prepare_bridge_install`共同纯准备Config/Bridge/Record，Bridge prepare成功返回前已经完成打包资源PE/hash/piped验证。Prepared落盘后只执行一次三目标optimistic precondition check，然后创建/验证全部staging并通过`all-staging-ready`。之后先执行Bridge/Record原子操作并持久化BridgeApplied，必要时启动临时Runtime，再执行Config原子操作并持久化ConfigApplied；Config应用前只核对仍持有的Bridge/Record Lease，不重跑optimistic precondition check，检查后的late modification由ReplaceFileW snapshot、no-replace publish或tombstone语义捕获。action-specific invariant使用Lease稳定的Handle字节及其解析结果验证，成功后持有全部普通Lease持久化StructureCommitted，再释放Lease、用新cleanup Handle清理、发布ListeningStatus并运行self-check。临时Runtime在self-check完成前收到的真实事件只作为认证活动证据，不得令状态进入running；self-check完成后第一条新的当前generation真实认证Hook才可进入running。任一`CapturedLateModification`、`DestinationAppeared`、`ActiveArtifactHandleConflict`、原子操作失败或Handle摘要错误立即停止后续文件，保留Journal/current target/capture/prepared backup并只允许安全retry。StructureCommitted之后self-check失败可派生partial/service_error；cleanup失败只产生Warning并保留StructureCommitted Journal，不能改变已提交结构的监听状态或回滚Hook/Bridge。

action-specific invariant在StructureCommitted前固定检查：Install要求CodePulse Hook按预期结构存在且原用户Hook保留，stable Bridge版本/摘要/PE属性正确，installation record与最终结构一致，并只确认Runtime请求意图而不因尚无真实事件声明running；Repair还要求缺失/旧Bridge与CodePulse Hook已修复、非CodePulse Hook和用户自有条目未被破坏或误删；Uninstall要求只移除CodePulse拥有的Hook、用户其他Hook保持、stable Bridge/record符合既定卸载目标，并明确文件事务完成不等于Runtime停止、空快照和UI状态传播已经完成。

失败边界固定为：Prepared前失败没有正式Journal、正式staging或稳定目标副作用，可直接返回；Prepared后首个原子操作前失败保留Journal但稳定目标未改变，只清理当前transactionId可证明的普通staging或交startup recovery，不做结构回滚；原子应用中失败同时依据Journal phase与Handle验证的真实artifact身份恢复，不盲信phase或路径；StructureCommitted后cleanup失败不回滚，只返回Warning并保留Journal供后续安全cleanup。Conflict artifact与cleanup Warning是不同结果：前者保留外部变化和证据，只能安全retry；后者表示结构正确但普通清理未完成，不得映射为整体安装失败。
...
它与apply共用integration operation mutex。没有稳定Journal且没有孤立Journal temp时返回`NoPendingTransaction`；其余情况只重跑`recover_interrupted_codex_integration_transaction()`。StructureCommitted前对target/replaced snapshot/conflict-preserved-current/tombstone/prepared backup重新获取普通Lease并通过Handle分类，任一应存在文件的Lease失败继续`IntegrationTransactionConflict`且Runtime停止；StructureCommitted后只使用新取得的cleanup Handle完成普通清理，失败返回Warning而不是Conflict。任何分支都不强制删除Journal、不忽略摘要、不把用户新字节采用为Original、不改Feature键。成功恢复后重新静态Inspection、按Inspection决定Runtime启停并发布最新ListeningStatus；仍有外改或活跃句柄时字节不变。用户手工把文件恢复为Journal已知Original/Target/ExpectedAbsent后可再次重试，安全完成回滚、cleanup或在invariant通过时提升StructureCommitted，无需重装。
...
7. 卸载在`all-staging-ready`通过后，先把稳定Bridge和安装记录按既定策略原子捕获为本transactionId的removed tombstone并持久化`BridgeApplied`，再精确移除CodePulse拥有的配置条目并持久化`ConfigApplied`；通过Uninstalled invariant并推进`StructureCommitted`后才清理墓碑。应用升级带入的新资源在下次启动修复稳定副本；旧安装包不会覆盖用户配置。
...
- 每次真正创建 HTTP/Actor Runtime 先从进程级 AtomicU64 分配非零 generation，并为该 generation 创建 token、DiscoveryOwner、Actor/reporter。新 generation 先清空旧认证事实；临时Runtime在StructureCommitted、cleanup尝试、状态发布与self-check完成前收到的真实事件只记录为认证活动，不能进入running。self-check完成后第一条新的当前generation真实事件才令监听状态进入running。旧 Actor 晚到上报、旧 HTTP 关闭回调和旧 DiscoveryGuard 都不能改变新 Runtime。
...
- Uninstall严格复用第3.6节顺序：先把稳定Bridge/记录原子捕获到transaction-owned tombstone并持久化BridgeApplied，再精确删除CodePulse marker并持久化ConfigApplied，通过Uninstalled invariant后持久化StructureCommitted。普通Lease释放和cleanup Handle尝试完成后，Runtime编排再按“停止接收 → 关闭 Actor/HTTP → Store.clear/发布空快照 → 发布 not_installed”停止链路并owner-aware删除discovery。StructureCommitted前的捕获/Lease/摘要问题是Conflict；StructureCommitted后的cleanup失败只Warning，不恢复Hook、不重启服务。本地hooks=false且有安全marker允许此路径且不先启动Runtime；managed disabled与ambiguous conflict禁止。
...
| 崩溃、并发外改、post-write或self-check失败留下不完整结构 | transactionId先于pure prepare；Prepared前零staging；三目标optimistic precondition check；verified staging与all-staging-ready；Bridge/Record→BridgeApplied→Config→ConfigApplied；existing `ReplaceFileW` snapshot、absent no-replace publish、removal tombstone；逐文件恢复与action-specific invariant | late modification由snapshot捕获且优先于prepared backup；DestinationAppeared不覆盖；Install/Repair不得提交悬空引用；Uninstall的BridgeApplied中间态不发布running，并由同一Journal恢复；Conflict可安全retry但无force |
...
| `features.codex_hooks` 弃用别名 | 04A 任务 1–3、04B-3、04C 任务 1/2/4 | 缺失/单键/同值/冲突/非布尔矩阵；弃用与重复 Issue；冲突时 Runtime 停止且三动作无内存变更计划或Prepared Journal；UI 无动作按钮 |
...
| Integration Transaction跨进程提交边界 | 04B-1/2、04C任务4 | 用户确认→allocate ID→pure prepare三资源→persist Prepared→三资源optimistic precondition check→verified staging→all-staging-ready→Bridge/Record atomic→BridgeApplied→Config atomic→ConfigApplied→action-specific invariant→StructureCommitted；最后一次检查之后的外改由snapshot/tombstone捕获；提交后只清理普通artifacts |
...
- “正常运行”只由StructureCommitted、普通cleanup尝试、状态发布与self-check完成后的第一条新真实认证Hook事件触发；之前临时Runtime收到的事件只证明认证活动。模拟HTTP自检不能替代信任或真实端验收。
...
- Install/Repair只有`InstalledOrRepaired` invariant（Marker=Exact且用户其他Hook保留、Bridge/Record=Target、版本/摘要/PE有效、记录与结构一致且无late snapshot冲突）通过后才能持久化`StructureCommitted`；Uninstall只有`Uninstalled` invariant（只移除CodePulse Hook、用户其他Hook保持、Marker=Absent、无稳定路径引用、Bridge/Record目标absent且tombstone受当前Journal控制）通过后才能提交。Install/Repair提交前不得留下悬空Hook；Uninstall的BridgeApplied中间态不得发布running。之后self-check失败不得删除正确结构，cleanup失败只Warning，恢复只清理普通artifact。
