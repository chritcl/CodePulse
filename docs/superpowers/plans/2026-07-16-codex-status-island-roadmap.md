...
它与apply共用integration operation mutex。没有稳定Journal且没有孤立Journal temp时返回`NoPendingTransaction`；其余情况只重跑`recover_interrupted_codex_integration_transaction()`，不强制删除Journal、不忽略摘要、不把用户新字节采用为Original、不改Feature键。成功恢复后重新静态Inspection、按Inspection决定Runtime启停并发布最新ListeningStatus；仍有外改时继续`IntegrationTransactionConflict`/`OrphanTransactionConflict`且字节不变。用户手工把文件恢复为Journal已知Original/Target/ExpectedAbsent后可再次重试，安全完成回滚、清理或在invariant通过时提升StructureCommitted，无需重装。
