...
- `StructureCommitted`：保留已提交目标；正常artifact cleanup不再采用“按路径重读摘要→调用路径型`DeleteFileW`”。先确认stage已持久化并释放对应普通Lease，再对每个非Conflict prepared backup/正常snapshot/已提交tombstone/temp调用`delete_verified_artifact_by_handle(path, expectedDigest)`：以`CreateFileW(GENERIC_READ | DELETE, FILE_SHARE_READ, OPEN_EXISTING)`取得排他性清理Handle，通过同一Handle取得file ID并计算摘要，匹配后用`SetFileInformationByHandle(FileDispositionInfo)`标记同一对象删除，关闭Handle并验证路径absent。Handle取得失败、delete pending或关闭后路径被新file ID复用只返回Warning并保留StructureCommitted Journal供下次startup recovery，不回滚Hook/Bridge；Conflict artifact永远不得进入普通cleanup。
...
| StructureCommitted清理在摘要检查后删除了同名新file ID | 正常artifact只用`CreateFileW(GENERIC_READ | DELETE, FILE_SHARE_READ)`取得清理Handle；同一Handle验证file ID/摘要后`SetFileInformationByHandle(FileDispositionInfo)` | Handle失败/delete pending/路径复用只Warning并保留StructureCommitted Journal；不调用路径型`DeleteFileW`，不回滚已提交结构；Conflict artifact永不进入普通清理 |
...
| 标准 JSON/TOML Hook Fixture | 04A任务1/3、04B任务4、04C任务4 | 两份母版完整八事件；JSON serde_json AST/TOML toml_edit AST注入；normalize接口显式接收`paths.installed_bridge`；CodePulse-owned projection排除用户独立Hook，混合group Repair保留用户handler；wrong-path/双command错误/extra参数非Exact；无原始文本replace |
...
| StableArtifactLease与Handle identity | 04B任务1/2/4、04C任务4 | `CreateFileW`只共享读；FileIdInfo volume serial+128位file ID+size；关键摘要只读Handle；ReplaceFileW后target/snapshot、Absent target、Removal tombstone均冻结；Lease阻止新write/rename/delete并覆盖writable mapping |
| Handle-based StructureCommitted cleanup | 04B任务1/4、04C任务4 | `GENERIC_READ | DELETE`+仅`FILE_SHARE_READ`；同一Handle摘要/file ID验证与`FileDispositionInfo`删除；路径复用不误删；Handle失败只Warning并由startup recovery重试 |
...
- [ ] **审核 04A：只读 inspection 与纯 planner**
  ...
  执行 `docs/superpowers/plans/2026-07-16-codex-status-island-04a-inspection-planner.md`。预期TempDir零写入、Inspection无动态字段、Feature标准键/弃用别名事实与冲突矩阵通过、标准JSON/TOML Fixture通过AST注入，Exact显式接收`paths.installed_bridge`并只比较CodePulse-owned projection；wrong-path/extra参数不可能Exact，独立用户group不影响Exact、混合group Repair保留用户handler；generation listening派生通过、本地disabled只允许安全uninstall、无writer/installer/UI；完成后停止review。

- [ ] **审核 04B：writer、installer、commands 与 runtime 生命周期**

  仅在04A已批准后执行`docs/superpowers/plans/2026-07-16-codex-status-island-04b-writer-installer.md`。预期统一Integration Journal四阶段、transactionId在Config/Bridge prepare前分配、Prepared前零staging；optimistic precondition check、prepared backup/temp与all-staging-ready通过；existing `ReplaceFileW`后target/snapshot Lease、absent publish后target Lease、removal后tombstone Lease、late modification两次ReplaceFileW后重新冻结、Lease跨Journal阶段持有及Handle-based cleanup通过；active writer/writable mapping/delete access进入可重试Conflict，稳定Journal/孤立temp、逐文件混合恢复、action-specific invariant、PE Machine+GUI、Store/generation/Owner、安全retry/NoPendingTransaction通过且无设置页；完成后停止review。

- [ ] **审核 04C：设置、显示偏好与 E2E**

  仅在04B已批准后执行`docs/superpowers/plans/2026-07-16-codex-status-island-04c-settings-e2e.md`。预期静态/动态UI分离、弃用别名提示、Transaction Conflict只显示“重新尝试安全恢复”、disabled marker卸载、重装revision/generation、原子filesystem九个精确注入点、活跃writer/writable mapping、Lease阻止新write/rename/delete、ReplaceFileW双Lease/no-replace target Lease/tombstone Lease/第二次恢复重新冻结、Handle-based cleanup与失败Warning、精确孤立Journal、Bridge/记录混合恢复、安全retry，实际安装结果经同一AST loader绑定稳定Bridge路径提取projection后与标准Fixture语义相等、Discovery竞态、GUI无闪窗和App真实信任通过；CLI环境阻塞如实记录；完成后停止review。
