...
**架构：** 静态inspection、预览和用户确认完成后，apply coordinator才分配transactionId，再让writer和installer pure prepare `ConfigApplyTransaction`/`BridgeInstallTransaction`及Record材料；完整Prepared Journal原子持久化前不创建正式staging、不修改稳定目标。Prepared后的三文件摘要重读统一称为optimistic precondition check，只负责验证用户确认时看到的目标未改变；通过后才创建/验证prepared backup、target temp和removal staging，`all-staging-ready`守住首个原子操作。之后统一执行Bridge/Record原子操作→BridgeApplied→必要临时Runtime→Config原子操作→ConfigApplied→action-specific invariant→StructureCommitted。existing目标由`ReplaceFileW`捕获实际旧内容，absent目标由no-replace发布，Uninstall把Bridge/Record原子rename为removed tombstone；原子操作后立即以仅共享读的`StableArtifactLease`冻结实际target与capture，关键摘要和身份只从Handle取得，全部普通Lease保持到StructureCommitted落盘。Journal只使用Prepared、BridgeApplied、ConfigApplied、StructureCommitted四阶段且不序列化Lease。StructureCommitted后先释放全部普通Lease，再使用新取得的cleanup Handle清理普通artifact；cleanup失败只Warning，不回滚或变成Conflict。startup orchestrator严格执行统一恢复→静态inspection→runtime generation决策→状态发布；stop/uninstall时清空Manager持有的唯一进程级`CodexSnapshotStore`。Vue设置页不在本批次实现。
...
- 卸载与其他action使用相同四阶段：先原子捕获Bridge/Record并持久化BridgeApplied，再精确移除CodePulse marker并持久化ConfigApplied，通过Uninstalled invariant后持久化StructureCommitted；Runtime停止、高revision空快照与not_installed发布属于之后的Runtime编排。
...
- 正式顺序统一为静态检查→展示预览→用户确认→分配transactionId→pure prepare Config/Bridge/Record且零稳定目标副作用→持久化Prepared→三目标optimistic precondition check→创建并验证prepared backup、target temp与removal staging→all-staging-ready→Bridge/Record原子操作→BridgeApplied→必要临时Runtime→Config原子操作→ConfigApplied→action-specific invariant→StructureCommitted→释放普通事务Lease→新cleanup Handle清理普通事务产物→发布awaiting_trust/partial/not_installed/service_error→本地self-check→self-check完成后当前generation首条新真实认证Hook→running；禁止确认前分配ID、prepare或写Journal。
...
- 普通`StableArtifactLease`、`StableArtifactIdentity`与`AppliedArtifactLeases`只属于当前进程内事务句柄，不得derive Serialize/Deserialize，不进入Journal、不成为第二套状态源；稳定target、replacement结果、正常snapshot、tombstone等普通Lease固定以`GENERIC_READ | FILE_READ_ATTRIBUTES`、仅`FILE_SHARE_READ`、`OPEN_EXISTING`取得并全部持有到StructureCommitted落盘。StructureCommitted后释放全部普通Lease；cleanup必须为每个普通artifact新开`GENERIC_READ | DELETE`、仅`FILE_SHARE_READ`、`OPEN_EXISTING`的Handle。禁止给普通Lease增加`DELETE`、复用它清理或把两者描述为同一Handle/生命周期。
...
三个审核点复用同一`integration_transaction_file`、同一`Prepared → BridgeApplied → ConfigApplied → StructureCommitted`状态机、同一SnapshotStore和同一组固定公开接口；不得借审核点创建第二套Journal、动态phase、状态机、Store或命令。`all-staging-ready`、precondition、cleanup与rollback都只是内存流程/恢复分支；`awaiting_trust`、`partial`、`not_installed`、`service_error`、`running`以及revision/generation都不是Journal phase。

既有公开类型名`PreparedCodexHookChange`保持不变；它只表示04A的纯内存变更计划，不表示Prepared Journal已经落盘，也不增加持久phase。
...
`AppliedArtifactLeases`由04B-1提供，供04B-2的进程内coordinator handle消费，不derive序列化。`target=Some`是Existing/Absent成功结果的硬约束；Removal后原路径必须absent，因此`target=None`且`removed_tombstone=Some`是唯一合法结果。所有原子结果普通Lease都持有到StructureCommitted持久化；BridgeApplied/ConfigApplied阶段写入失败时不释放Lease并继续其他目标，而是保留current target/capture/prepared backup/Journal并返回`IntegrationTransactionConflict`。StructureCommitted持久化成功后先释放全部普通事务Lease，再进入cleanup Handle流程；进程崩溃由OS关闭Handle，recovery重新获取普通Lease，Journal仍是唯一跨进程事实。Runtime启停、self-check和公开状态映射全部留给04B-3。
...
transaction pure prepare 是严格无稳定目标副作用阶段：允许解析校验输入，读取Config/Bridge/Record，计算存在性/身份/摘要、目标字节、安装记录和确定性staging/capture路径，验证打包Bridge的PE/hash/piped契约并把材料保存在内存；禁止修改稳定Bridge、安装记录、Hook配置，禁止创建prepared backup/target temp/snapshot/tombstone/Journal或启动Runtime。coordinator构造含全部路径/摘要的Journal并成功持久化Prepared后，先执行一次三文件 **optimistic precondition check**：重新读取Config、Bridge、Record，分别调用`verify_artifact_matches_expected_original()`按Journal的existedBefore/originalDigest/targetDigest分类，只有三者都为Original/ExpectedAbsent才创建正式staging。Target或ExternalModification返回`ConcurrentModification`（恢复场景映射稳定Conflict）、保留Journal、不采用变化后的字节作为新Original，且不创建新staging、不修改目标。该检查验证稳定目标是否仍与预览一致；随后staging verification验证事务artifact是否符合计划摘要，两者不得混称或调换。检查只能提前发现修改，不能保证后续Windows原子操作前没有变化，禁止称为原子Compare-And-Swap。

每个existedBefore=true的prepared backup必须从当前目标重新读取原始字节，计算摘要并与Journal.originalDigest相等后，才以`create_new(true)`创建；随后write→flush→close→取得普通事务Lease并通过同一Handle核对identity与originalDigest，该Lease持有到StructureCommitted。existedBefore=false时prepared backup必须为None且目标仍不存在；此时若目标出现，按ExternalModification处理。所有target temp同样在close后通过Handle验证identity与targetDigest。用于验证尚待rename的target temp的Handle是`all-staging-ready`前的短生命周期staging verification Handle，验证结束后关闭以允许原子rename，不得复用或冒充原子操作后持有到StructureCommitted的普通事务Lease；原子操作完成后必须获取结果普通Lease。targetExists=false的Uninstall不创建空payload temp，而是验证removed tombstone目的地缺失、同目录/同卷且由transactionId归属，不能用零字节文件冒充absent。

第一个稳定目标原子操作前存在不可绕过的`all-staging-ready`内存屏障：Config/Bridge/Record的prepared backup/temp（或验证后的removal无payload状态）全部创建并通过Handle-based identity/摘要验证；所有需要保留的prepared backup、snapshot/tombstone/capture路径的存在性、归属、同目录/同卷条件满足；重新读取的稳定Prepared Journal仍是当前transactionId。任何资源未就绪、摘要错误、捕获路径意外出现或Journal ID/stage不符都不得开始，且不得新增AllStagingReady Journal phase。

`AtomicIntegrationFs`必须提供三种不同接口，writer与installer共同调用，禁止通用`move_with_replace(...)`：

- `atomically_replace_existing_artifact()`只处理`existed_before=true && target_exists=true`。统一optimistic precondition check与`all-staging-ready`已经在coordinator中完成；单文件接口不得重跑precondition，只验证replacement temp/Journal前置事实后调用`ReplaceFileW(target, replacement, transaction_owned_replaced_snapshot, ...)`。成功后的固定顺序是`BeforeStableLeaseAcquire`→获取target Lease→获取snapshot Lease→`AfterStableLeaseAcquireBeforeHash`→用两个Handle分别计算摘要→验证volume serial/file ID组合不同→`AfterHashBeforeStagePersist`。只有target=`targetDigest`、snapshot=`originalDigest`且两个Lease均成功才返回`AppliedExpectedOriginal`；任一Lease失败返回`ActiveArtifactHandleConflict`，不按路径读摘要、不清snapshot、不继续下一文件。snapshot稳定但不等于originalDigest才返回`CapturedLateModification`。
...
- `atomically_capture_artifact_for_removal()`提供通用removal基础。统一optimistic precondition check与`all-staging-ready`已经完成后，把target no-replace原子rename到本transactionId removed tombstone；单文件接口不得重跑precondition，且禁止`DeleteFileW`。成功后获取tombstone Lease、只通过Handle计算expectedDigest，并在Lease持有时验证原target absent。旧写句柄继续指向tombstone、writable mapping未释放或delete access仍活动时Lease失败；此时不按路径读取、不永久删除，保留Journal等待安全retry。`Uninstalled` invariant属于04B-2 coordinator。
...
- [ ] **步骤 1：先写摘要竞争、Journal 捕获路径与 prepared backup 失败测试**
  ...
  用通用artifact fixture覆盖expectedDigest/previewDigest不符与任一目标变化；每例断言用户确认前transactionId/prepare/Journal/staging调用均为零，且不创建temp/prepared backup/snapshot/tombstone。成功路径验证确认后transactionId在pure prepare前分配并进入唯一Journal，不在04B-1实现或调用`ConfigApplyTransaction`、`BridgeInstallTransaction`。Prepared已持久化后先运行且只运行一次三目标optimistic precondition check；固定竞态为任一目标在staging前变化时不创建新staging、不修改目标。该检查只用于提前发现修改，不把它当作最后时刻竞态的完整证明，也不与staging verification混称。
...
- [ ] **步骤 3：先写九组固定崩溃恢复测试**
  ...
  3. BridgeApplied：artifact B=Target、artifact C=Original → 只恢复稳定Target，Original保持；Uninstall中B=tombstone、C仍稳定时输出同一阶段的稳定capture事实供04B-2恢复；
  4. BridgeApplied：artifact B=Original、artifact C=Target → 只恢复稳定Target，Original保持；Uninstall反向部分tombstone同理；
  5. BridgeApplied：artifact B=Target、artifact C=ExpectedAbsent → 只处理本事务Target；反向混合同理；不在04B-1判断Install/Repair/Uninstall业务语义；
  ...
  removal基础另固定四组：artifact rename到tombstone后旧writer仍活动，tombstone Lease失败且不永久删除；tombstone后目标路径被重新创建，两者保留并Conflict；Uninstall Bridge/Record捕获成功但BridgeApplied尚未落盘时按真实Handle识别阶段滞后；普通tombstone与prepared backup Lease保持到StructureCommitted后先释放普通Lease，再使用新取得的cleanup Handle删除。真实Uninstall coordinator与`Uninstalled` invariant留到04B-2。
...
`prepare_config_apply()`与`prepare_bridge_install()`都是pure prepare，只验证/计算Config、Bridge、Record目标材料、原/目标摘要及由transactionId确定的temp/prepared backup/replaced snapshot/conflict-preserved-current/removed tombstone路径，并把待写字节保存在内存；`prepare_bridge_install()`成功返回前完成打包Bridge PE/hash/piped验证。二者禁止创建正式artifact、替换稳定目标、启动Runtime或创建Journal。调用方用相同transactionId构造并持久化完整Prepared Journal后，由同一coordinator先完成且只完成一次三文件optimistic precondition check，再创建/验证staging并通过`all-staging-ready`，才调用两个`apply()`。writer与installer只能调用04B-1统一existing/absent/removal原子接口，不得各自实现替换或重跑precondition。任一`CapturedLateModification`、`DestinationAppeared`、`ActiveArtifactHandleConflict`或Handle摘要失败都停止，不继续下一文件，并按Lease稳定的实际target/capture action-aware恢复。Bridge/Record完成后保持Lease推进BridgeApplied，Config完成后保持全部普通Lease推进ConfigApplied与StructureCommitted；两个`commit()`只终结进程内资源。StructureCommitted落盘后释放全部普通事务Lease，并使用新取得的cleanup Handle完成普通artifact清理；Conflict artifacts保留。
...
- [ ] **步骤 2：先写Config Writer、installer与action coordinator失败测试**
  ...
  覆盖取消/关闭确认框/确认前摘要失败时transactionId、两个prepare、Journal和staging调用均为零；确认后transactionId先分配，两个pure prepare只产生Config/Bridge/Record摘要、目标字节和确定性路径且文件系统零写入，Bridge prepare返回前完成PE/hash/piped验证。Prepared后先optimistic precondition check，再创建本ID prepared backup/temp/removal staging并通过`all-staging-ready`。已有Config/Bridge/Record必须由ReplaceFileW捕获snapshot并取得target/snapshot双Lease；首次创建必须no-replace publish并取得target Lease。正常existing路径snapshot Handle=originalDigest、target Handle=targetDigest且file ID不同；最后一次检查之后、系统原子替换内部之前修改任一目标时snapshot捕获late modification且不继续。Config/Bridge handle的transactionId用`assert_eq!`固定相同；Install、Repair、Uninstall都断言Bridge/Record→BridgeApplied→Config→ConfigApplied→invariant→StructureCommitted，且普通Lease到StructureCommitted前不释放。
...
- [ ] **步骤 3：先写原子应用回滚与提交后self-check失败测试**

  pure prepare阶段对打包Bridge执行`--codepulse-self-check`/Hook管道契约验证，失败时没有Prepared Journal、正式staging或稳定目标副作用。稳定目标原子应用过程中再注入Bridge已替换/记录仍旧、Bridge仍旧/记录已替换、Config应用失败等场景，断言按Journal phase和真实Handle身份只回滚稳定Target artifact。提交后调用记录固定断言`StructureCommitted`持久化→释放普通事务Lease→为普通产物取得新cleanup Handle并尝试清理→发布awaiting_trust/partial/not_installed/service_error→运行本地self-check；self-check无法启动、超时或输出违约时保留已经正确提交的Hook/Bridge/Record，只派生partial/service_error，不执行逐文件回滚。self-check完成前收到的真实事件不得进入running，完成后下一条新的当前generation真实认证Hook才可进入running。

  精确竞态专例固定为：`AfterTargetOpenedOrPrepared`修改Bridge，ReplaceFileW snapshot捕获late modification；Bridge正常Target后Record替换瞬间捕获late modification或Record Lease失败，不继续Config且Repair只使用稳定Lease恢复Bridge；snapshot旧写Handle仍活动时不执行二次ReplaceFileW，关闭Handle后retry读取最终digest；late snapshot且当前target仍为CodePulse Target时释放第一次双Lease、二次ReplaceFileW、再获取restored/conflict-current双Lease；第二次后任一活跃写Handle时不清两版本。任一原子操作报告成功但Handle目标摘要不是targetDigest，不继续记录或配置。文件占用与Lease取得重试固定3次、间隔50ms，测试注入sleeper不真实等待；不得在staging或单文件apply阶段重跑optimistic precondition check，安全性由统一早期检查、原子操作捕获结果和后置Lease共同保证。
...
- [ ] **步骤 4：实现Config Writer、PE校验、installer与action coordinator**
  ...
  Rust解析顺序与PowerShell完全一致：DOS Header≥64字节；MZ；e_lfanew；PE签名；COFF Machine；Optional Header与WindowsGui Subsystem。writer以04A AST目标字节pure prepare`ConfigApplyTransaction`，installer在pure prepare内构造Bridge/Record材料并完成metadata、SHA-256与piped契约验证；Prepared前不得创建正式staging或替换稳定路径。Prepared后的三目标precondition、staging verification与`all-staging-ready`由统一coordinator按固定顺序提供，两者只调用统一原子接口并保存结果Lease。Install/Repair/Uninstall全部固定Bridge/Record→BridgeApplied→必要临时Runtime→Config→ConfigApplied→action-specific invariant→StructureCommitted；rollback按action和稳定capture优先级逐文件恢复。StructureCommitted后释放全部普通Lease，再使用新取得的cleanup Handle清理普通artifact；cleanup失败只Warning，不回滚已正确提交结构。
...
- [ ] **步骤 1：先写 disabled action matrix 命令测试**
  ...
  固定序列：inspection hooksFeature=disabled + marker absent → preview install 返回 HooksDisabled →变更计划不存在→transactionId/pure prepare/Journal/staging/installer/writer/ensure_started计数均为0→路径快照不变；disabled + marker present + repair 同样HooksDisabled。disabled + 安全marker exact/modified/duplicate + uninstall → preview/confirm/apply成功，不启动Runtime或安装新Bridge；统一coordinator先捕获稳定Bridge/Record并推进BridgeApplied，再由writer只删除CodePulse marker且其他Hook深度相等，推进ConfigApplied/invariant/StructureCommitted后清理普通tombstone。用户手动把hooks改为true并重新inspect后install/repair preview才成功。只有旧`codex_hooks`时按effectiveState执行并返回弃用Issue；两个Feature键冲突或任一非布尔时三action全部ConfigConflict，计划/transactionId/Journal/staging/writer/runtime计数均为0。
...
- [ ] **步骤2：先写命令委托04B-2 coordinator与Runtime回调测试**
  ...
  04B-3不再实现下列事务步骤；命令测试注入04B-2已审核coordinator的调用记录，只断言preview/confirm/apply将inspection/双摘要和同一operation mutex交给它，并按其Runtime请求意图执行ensure_started/stop/clear、ListeningStatus与self-check。记录仍必须完整呈现唯一顺序以防命令层绕开coordinator：

  ```text
  1. 重新静态inspection并展示preview
  2. 用户确认；取消/关闭时后续调用全部为零
  3. apply锁内重新计算expectedDigest/previewDigest
  4. 分配唯一transactionId
  5. 用同一transactionId pure prepare Config、Bridge、Record；Bridge prepare返回前完成PE/hash/piped验证
  6. 构造并原子持久化包含三类目标预条件、目标摘要与确定性路径的Prepared Journal
  7. 重新读取Config/Bridge/Record执行一次optimistic precondition check；任一变化时不创建正式staging
  8. 创建并通过Handle验证prepared backup、target temp、removal staging，再验证`all-staging-ready`
  9. 原子应用Bridge/安装记录，获取并保存结果普通Lease
  10. 保持全部普通Lease，把同一Journal推进BridgeApplied
  11. 必要时ensure_started临时Runtime；self-check前事件不得进入running
  12. 原子应用Config，不重跑optimistic precondition check，获取并保存结果普通Lease
  13. 保持全部普通Lease，把同一Journal推进ConfigApplied
  14. 使用Lease稳定的Handle字节及解析结果执行action-specific invariant
  15. 保持全部普通Lease，把同一Journal推进StructureCommitted
  16. 释放全部普通事务Lease，commit进程内handle，再使用新取得的cleanup Handle清理普通artifact
  17. 派生并发布awaiting_trust、partial、not_installed或service_error
  18. 运行完整self-check
  19. self-check完成后立即返回当前inspection/listeningStatus/selfCheck结果；Runtime状态机异步等待下一条新的当前generation真实认证Hook，并通过权威listening event进入running
  ```

  调用记录器必须专门断言用户确认前transactionId/prepare/Journal/staging为零，确认后transactionId在`prepare_config_apply`/`prepare_bridge_install`前分配且两个prepare收到同一ID；Prepared原子写入前任何故障都无稳定目标变化、无正式staging。Prepared写入后、staging前先且只先对三目标执行一次optimistic precondition check；任一变化都不创建新staging。prepared backup/temp创建一半崩溃时，下次恢复只清本transactionId普通staging，不删除并存的另一ID文件；全部staging完成但任一摘要错误、capture路径意外存在或Prepared Journal不再是当前ID时不开始第一个原子操作。
...
- [ ] **步骤3：先写coordinator结果、Runtime生命周期与Uninstall命令映射测试**
  ...
  再覆盖安装生命周期：临时generation=1在ConfigApplied前、StructureCommitted前或self-check期间收到真实事件都只记认证活动且不running；self-check完成后下一条新的generation=1真实认证事件才running。确认卸载事务StructureCommitted并完成cleanup尝试后→generation=None/authenticatedGeneration=None/更高revision空快照/not_installed；重新安装→generation=2、awaiting_trust，绝不能沿用generation=1直接running；generation=1晚到reporter忽略；generation=2同样只在self-check后首条新真实事件进入running。
...
- `paths.integration_transaction_file` 是唯一 Journal；Writer/Installer 不拼接路径、不创建第二日志。用户确认前不分配transactionId、不prepare、不写Journal；确认后transactionId在Config/Bridge/Record pure prepare前分配，两个handle收到同一ID。prepare只计算/验证内存材料且零稳定目标/正式staging副作用，Bridge prepare返回前完成打包资源PE/hash/piped验证。Journal不含正文、Token、Hook输入、用户命令、项目路径正文或Bridge bytes。
...
- Install/Repair/Uninstall按唯一顺序执行：静态检查→展示预览→用户确认→分配transactionId→pure prepare三资源→持久化Prepared→三资源precondition→创建并验证全部staging→all-staging-ready→Bridge/Record原子操作→BridgeApplied→必要临时Runtime→Config原子操作→ConfigApplied→invariant→StructureCommitted→释放普通事务Lease→新cleanup Handle清理→发布集成/监听状态→本地self-check→self-check后首条新真实认证Hook→running。InstalledOrRepaired要求Marker=Exact且用户Hook保留、Bridge/Record=Target、版本/摘要/PE有效、记录与结构一致；Uninstalled要求只移除CodePulse Hook、用户其他Hook保持、Marker=Absent、无稳定路径引用、Bridge/Record目标absent且tombstone受当前Journal控制。StructureCommitted后self-check失败保留正确结构并返回partial/service_error；cleanup失败只Warning，不改变监听状态或整体安装结果。
...
- 首次Install在StructureCommitted前失败时停止临时Runtime、发布更高revision空快照并owner-aware删除discovery；Repair失败且安全恢复后保持原合法链路。Uninstall先按Bridge/Record→BridgeApplied→Config→ConfigApplied→invariant→StructureCommitted提交文件结构并尝试cleanup，之后Runtime编排才发布空快照和not_installed；文件事务完成不等于UI状态传播已经完成。
...
- 范围脚本必须阻止snapshot/tombstone路径型关键摘要、ReplaceFileW后缺target/snapshot Lease、no-replace后缺target Lease、removal后缺tombstone Lease、recovery在Lease失败后降级路径读取、StructureCommitted直接路径型DeleteFileW、Conflict artifact被普通cleanup删除，以及普通事务Lease在StructureCommitted前释放。
