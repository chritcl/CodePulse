...
- `StableArtifactLease`只存在于04B当前进程的普通事务句柄，不序列化、不写Journal、不改变唯一Journal/唯一SnapshotStore/Runtime generation/DiscoveryOwner所有权；它以`GENERIC_READ | FILE_READ_ATTRIBUTES`、仅`FILE_SHARE_READ`取得并持有到对应阶段或StructureCommitted落盘，崩溃恢复重新获取Lease。StructureCommitted后必须先释放普通Lease，再新开`GENERIC_READ | DELETE`、仅`FILE_SHARE_READ`的cleanup Handle；禁止给原Lease增加`DELETE`权限或复用它删除。
