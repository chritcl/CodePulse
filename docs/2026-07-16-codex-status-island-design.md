...
多个活动配置层可能同时启动语义相同的 CodePulse Hook。Bridge 仍为每次投递生成随机 `eventId`，不扫描配置层、不维护持久状态；只有通过协议与认证校验、并被当前 Runtime generation 接受的事件才进入 Rust 单线程 Actor，随后先执行第一层 `eventId` 去重，再按事件级策略处理跨层重复。对于具有足够稳定标识的 Tool、Subagent 和 Turn 事件，Actor 使用第二层有界逻辑事件键消除重复投递；对于 `SessionStarted` 和 `PermissionRequested`，Actor 分别采用幂等元数据更新、等待状态转换幂等与提醒边沿抑制。缺乏无歧义稳定标识的事件不得使用内容或时间猜测补键，以免错误合并同一轮中的合法连续事件。
...
→ 用户确认
→ 分配 transactionId
→ 纯准备 Config、Bridge、Record 的事务材料，零稳定目标副作用
→ 持久化 Prepared Journal
→ 对 Config、Bridge、Record 执行 optimistic precondition check
→ 创建并验证 prepared backup、target temp 和 removal staging
→ 验证 all-staging-ready
→ 原子应用 Bridge 与安装记录并持久化 BridgeApplied
→ 必要时启动临时 Runtime
→ 原子应用 Hook 配置并持久化 ConfigApplied
→ 验证 action-specific invariant
→ 持久化 StructureCommitted
→ 释放普通事务 StableArtifactLease
→ 使用新取得的 cleanup Handle 清理普通事务产物
→ 发布 awaiting_trust、partial、not_installed 或 service_error
→ 执行本地自检
→ 等待当前 Runtime generation 的第一条真实认证 Hook 事件
→ 进入 running
```

`Prepared → BridgeApplied → ConfigApplied → StructureCommitted` 是唯一正式事务阶段；`all-staging-ready` 只是内存屏障，不是第五个阶段。`awaiting_trust`、`partial`、`not_installed`、`service_error`、`running` 是集成或监听状态，`revision`、`generation` 是状态版本事实，均不是文件事务阶段。配置写入成功不能直接显示 `running`；只有本地自检完成后，当前 Runtime generation 的第一条新真实认证 Hook 事件才能令状态进入 `running`。

`Prepared` 表示用户已经确认、transactionId 与事务意图已经冻结、当前稳定目标的存在性/身份/摘要预条件及目标内容和确定性路径已经记录、pure prepare 已完成；此时尚未创建正式 staging，也没有修改 Hook、Bridge 或安装记录。`prepare_config_apply` 与 `prepare_bridge_install` 若保留现名，只能计算并返回事务材料，不得创建正式 staging artifact 或修改稳定目标。Prepared 落盘后必须先检查三个稳定目标是否仍与预览一致，再创建和验证 staging；任一 precondition 不满足时不创建 staging、不覆盖外部变化，也不自动采用新字节继续。

只有 Config、Bridge、Record 所需 staging 全部完成 Handle-based identity/摘要验证、需要保留的 backup/snapshot/tombstone 条件均满足且当前 Journal 仍属于同一 transactionId，`all-staging-ready` 才通过。任一 staging 失败时零稳定目标操作。StructureCommitted 是唯一跨进程结构提交点：action-specific invariant 已满足后才可落盘；落盘后普通 cleanup 失败只产生 Warning并保留恢复证据，不回滚正确结构。Conflict artifact 不进入普通 cleanup，安全 retry 只能重跑检查，不能覆盖或删除外部变化。

普通事务 `StableArtifactLease` 在对应事务步骤取得，用于冻结稳定 target、原子操作后的 replacement 结果、snapshot、tombstone 及其他参与身份验证的普通 artifact，并全部持有到 `StructureCommitted` 成功持久化。之后先释放普通事务 Lease，再为每个普通事务产物取得新的 cleanup Handle；在同一 cleanup Handle 上依次读取 identity、计算摘要、再次确认 identity 并标记删除，关闭后才允许路径复用。普通事务 Lease 与 cleanup Handle 不是同一 Handle或同一生命周期，cleanup 不得按路径降级到 `DeleteFileW`。
...
HTTP 接收层完成请求体大小、JSON/协议结构和认证校验后，才把事件放入当前 Runtime 私有的有界队列；任一校验失败均在入队前返回，不得读取或写入 Actor 的 `eventId` 缓存。Actor 收到队列事件后还必须验证其捕获的 generation 仍是 Manager 接受的当前 generation，旧 Runtime、已失效 Runtime 或停止接收阶段的事件不得进入聚合器。
...
所有通过协议校验、认证校验，并被当前 Runtime generation 接受的事件，在产生任何任务状态副作用前，首先经过 Actor 独占的第一层 `eventId` 有界去重。第一层命中后立即停止：不执行第二层处理、不更新任务或活动时间、不产生 attention、不提交 Snapshot，也不分配 revision；只允许记录不含用户内容的重复事件 Debug 原因。第二层只接受能够用稳定标识无歧义构造键的事件：Tool 使用 `sessionId + turnId + eventType + toolUseId`，Subagent 使用 `sessionId + turnId + eventType + agentId`，Turn 使用 `sessionId + turnId + eventType`。稳定标识缺失时不进入第二层缓存，也不得使用 prompt、cwd、路径、命令、tool input/output、授权说明、内容摘要或 `occurredAt` 猜测补键。

```text
HTTP 请求接收 → 请求体大小与 JSON/协议结构校验 → 认证校验 → Actor 检查当前 Runtime generation → eventId 去重 → 事件级精确逻辑键或幂等处理 → 更新 Snapshot → SnapshotStore 分配并发布 revision
```

`SessionStarted` 不依赖第二层缓存：不同 `eventId` 的重复到达只执行幂等 session 元数据更新，不创建任务卡片、attention、提醒或子智能体计数。`PermissionRequested` 也不进入声称无歧义的逻辑键缓存；它只在任务发生 `非 waiting_approval → waiting_approval` 转换时创建新的强提醒。同一轮已经等待授权时，现有非敏感稳定字段无法区分跨配置层重复投递与尚未解除等待状态的另一条合法授权投递，因此首版明确选择抑制重复 attention 和强提醒，只刷新安全活动时间和脱敏说明，不增加计数；这不是无损精确事件识别。后续有效工具事件、新轮次或 Stop 退出等待状态后，新的授权事件可以再次产生提醒边沿。不得用固定时间窗口无条件合并同一轮的全部授权请求。

两层有界缓存与上述事件级状态策略均由单线程 Actor 独占维护。Bridge 只生成随机 `eventId` 和最小事件，Vue 只消费权威快照，二者都不承担跨配置层逻辑去重。
...
- `PermissionRequest` 对应的 `PermissionRequested` 不进入第二层逻辑事件键缓存；
- 当前任务发生 `非 waiting_approval → waiting_approval` 时立即进入等待授权并强打断主岛；
- 同一轮已经处于 `waiting_approval` 时，重复事件不创建第二个 attention、不重复播放强提醒、不增加计数，只可刷新安全活动时间和脱敏后的操作说明；
- 不受 1 秒平滑限制；
- 不自动超时；
- 不提供允许/拒绝按钮；
- 优先显示授权说明；
- 无说明时根据脱敏命令生成摘要；
- 后续有效工具事件、停止或新轮次事件退出等待状态；退出后新的授权事件可以再次进入等待状态并产生新的强提醒；
- 不使用 `tool_input`、命令正文、prompt、cwd、路径、授权说明正文、内容摘要或 `occurredAt` 构造持久逻辑键，也不使用固定时间窗口无条件合并授权请求。
...
- 解析失败停止；
- 展示变更预览；
- 用户确认后分配 transactionId；
- 纯准备 Config、Bridge、Record 的事务材料，不创建正式 staging、不修改稳定目标；
- 持久化唯一 Prepared Journal；
- Prepared 后、staging 前依次对 Config、Bridge、Record 执行 optimistic precondition check；
- 检查重复条目。
...
创建并验证全部 staging
→ all-staging-ready
→ Bridge/Record 原子操作并持久化 BridgeApplied
→ 必要时启动临时 Runtime
→ Hook Config 原子操作并持久化 ConfigApplied
→ action-specific invariant
→ 持久化 StructureCommitted
```

precondition check 验证用户确认时看到的稳定目标仍未改变；staging verification 验证即将写入或保留的事务 artifact 与 Journal 计划摘要一致。两者不能合并或调换顺序。Prepared 前失败不留下正式 Journal/staging且无需恢复；Prepared 后首个原子操作前失败不做结构回滚；原子应用中失败按 Journal 阶段和真实 Handle 身份恢复；StructureCommitted 后 cleanup 失败只返回 Warning。

`StructureCommitted` 前的 action-specific invariant 固定为：

- Install：CodePulse Hook 按预期结构存在并保留原用户 Hook；stable Bridge 的版本、摘要和 PE 属性正确；installation record 与最终结构一致；必要 Runtime 请求意图明确；没有本地自检后的真实认证 Hook 事件时仍不得声明 `running`；
- Repair：缺失或旧 Bridge 已修复，CodePulse Hook 结构正确，非 CodePulse Hook 未被破坏或误删，installation record 与最终结构一致；
- Uninstall：只移除 CodePulse 拥有的 Hook，用户其他 Hook 保持，stable Bridge 与 installation record 按既定卸载策略处理；Runtime 停止、高 revision 空快照和完整状态传播仍属于提交后的 Runtime 编排。
...
- 保留其他 Hook；
- 不用旧备份覆盖用户完整当前配置；
- 按统一阶段先原子捕获 Bridge 与安装记录并持久化 BridgeApplied，再精确移除 CodePulse 配置引用并持久化 ConfigApplied；
- `Uninstalled` invariant 在 StructureCommitted 前验证只移除 CodePulse Hook、用户其他 Hook 保持、稳定 Bridge/记录符合卸载目标；Runtime 停止与高 revision 空快照仍由后续 Runtime 编排负责；
- `StructureCommitted` 落盘后释放普通事务 Lease，使用新取得的同一 cleanup Handle 完成身份校验、摘要校验和删除标记；
- cleanup 失败只保留 Warning、StructureCommitted cleanup Journal与待清理证据，不属于未提交事务或Conflict，也不回滚已正确提交的 Hook 或 Bridge；
- Conflict artifact 永不进入普通 cleanup，Conflict 与 cleanup Warning 分开报告；
...
新会话、新轮次覆盖、多会话汇总、优先级、最近活动排序、1 秒平滑、授权强打断、中间命令失败、Stop 保守判定、5 分钟完成删除、失败手动清除、10/30 分钟中断、授权不超时和乱序。事件去重专项覆盖：协议非法、未认证、旧 generation 和失效 Runtime 均不能污染 `eventId` 缓存；第一层重复不刷新活动、不产生 attention、不提交 Snapshot 或分配 revision；相同 `toolUseId`/`agentId`/`turnId` 且不同 `eventId` 的精确事件只处理一次，不同稳定 ID 不误合并；重复 `SessionStarted` 只幂等刷新元数据；同一轮重复 `PermissionRequested` 不产生第二次强提醒，退出等待状态后可以再次提醒；缺失稳定字段按事件级规则处理；逻辑键和 Debug 不含用户内容；两层缓存均有界；Bridge 与 Vue 不承担第二层去重。
...
自动测试覆盖十四项设计场景对应的内部行为和不变量：CLI/App 来源 DTO 的模拟输入、两端并行聚合、读取/修改/命令/测试阶段、授权、先失败后成功、最终失败、五分钟保留、失败清除、退出无历史补偿、端口 fallback、用户 Hook 保留、等待信任状态、Bridge 更新与卸载行为。它还覆盖协议、优先级、生命周期、文件事务、恢复、generation/revision，以及多配置层下稳定事件逻辑键与 Session/Permission 幂等策略。
